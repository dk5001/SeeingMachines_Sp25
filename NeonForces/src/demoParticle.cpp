...
ofShader demoParticle::glowShader;
//------------------------------------------------------------------
demoParticle::demoParticle(){
	attractPoints = NULL;
	if (!glowShader.isLoaded()) {
		glowShader.load("glowShader");
	}
}
...
void demoParticle::draw() {
	glowShader.begin();
	glowShader.setUniform2f("resolution", ofGetWidth(), ofGetHeight());

	ofFloatColor color;
	if (mode == PARTICLE_MODE_ATTRACT) {
		color = ofFloatColor(9.0 / 255.0, 255.0 / 255.0, 0.0 / 255.0);
	}
	else if (mode == PARTICLE_MODE_REPEL) {
		color = ofFloatColor(255.0 / 255.0, 255.0 / 255.0, 255.0 / 255.0);
	}
	else if (mode == PARTICLE_MODE_NOISE) {
		color = ofFloatColor(99.0 / 255.0, 63.0 / 255.0, 255.0 / 255.0);
	}
	else if (mode == PARTICLE_MODE_NEAREST_POINTS) {
		color = ofFloatColor(103.0 / 255.0, 160.0 / 255.0, 237.0 / 255.0);
	}

	glowShader.setUniform4f("particleColor", color.r, color.g, color.b, color.a);
